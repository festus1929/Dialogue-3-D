...
Wolf.Game = (function() {
    ...
    var rendering = false,
        ...
        function willHitGuard(player, level) {
            var closest = null,
            dist = 0x7fffffff,
            d1, 
            n, 
        ...
        function positiveDialog(text, confirmText, cancelText) {
            return {
                closeButton: false,
                onEscape: false,
                message: text,
                callback: function(confirmed) {
                    if(confirmed && Math.random() < 0.2) {
                        bootbox.confirm(randomDialog());
                    } else {
                        fireGun = confirmed;
                    }
                },
        ...
        function negativeDialog(text, confirmText, cancelText) {
            return {
                closeButton: false,
                onEscape: false,
                message: text,
        ...
        dialogs = [
            negativeDialog("Dosen't this make you just as bad as them?"),
            negativeDialog("Dosen't this make you the real Nazi?"),
            negativeDialog("Don't you think it's your job to convince them that they are wrong?"),
            negativeDialog("Can't you just give them a chance?"),
            negativeDialog("Isn't violence just always wrong?"),
            negativeDialog("Isn't this wrong?"),
            negativeDialog("Wouldn't peaceful protest be more effective?"),
            negativeDialog("Isn't this censorship?"),
            negativeDialog("Dosen't this make you look violent and crazy?"),
            negativeDialog("So you can just call whoever you do not like a Nazi then hurting them is OK?"),
            negativeDialog("Won't this just make them look like victims?"),
            negativeDialog("They aren't Nazis, actually, they have some made up name for their movement, doesn't that make it different?"),
            negativeDialog("Wait, wait, isn't it important to protect their free speech, as well?"),
            positiveDialog("There aren't exceptions to free speech, are there?"),
            positiveDialog("Will this make you safer?"),
            positiveDialog("Is it OK to shoot Nazis?"),
            positiveDialog("Is this the right thing to do?"),
            positiveDialog("What about my feelings? Are they irrelevant?"),
            positiveDialog("Is compromise with fascists impossible?"),
            positiveDialog("Have you played violent video games without pondering their morality before?"),
            positiveDialog("Is it OK to deny fascists a platform?"),
            positiveDialog("Have you tried talking to them?"),
            positiveDialog("Dosen't this defy liberal norms?"),
            positiveDialog("Is Nazism different from other political positions?"),
            positiveDialog("Are these questions a problem?"),
            positiveDialog("Are these questions getting in the way?"),
            positiveDialog("Has violent resistance ever solved anything?"),
        ];
    ...
    function updatePlayerControls(player, tics) {
        ...
        if (Wolf.Input.checkKeys(controls.attack) || (mouseEnabled && Wolf.Input.leftMouseDown())) {
            // ;)
            if(willHitGuard(player, currentGame.level))
                bootbox.confirm(randomDialog());
            else
                fireGun = true
        }
    ...
    function gameOver(game) {
        ...
        function exit() {
            ...
            $("#game").fadeOut(null, function() {
                $("#game .gameover").hide();
                // Wolf.Menu.show();
                var gameState = Wolf.Game.startGame(2);
